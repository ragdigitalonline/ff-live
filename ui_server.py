...
app = FastAPI(title="FlyingFish AI Dashboard")
...
def read_config():
    ...
    return {
        "first_line": get_val("first_line", "FIRST_LINE", "Namaste! This is Aryan from FlyingFish AI — we help businesses automate with AI. Hmm, may I ask what kind of business you run?"),
...
<head>
  ...
  <title>AI Voice Demo — FlyingFish AI</title>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .avatar{width:80px;height:80px;border-radius:50%;background:linear-gradient(135deg,#3dbf7f,#29b6e8);display:flex;align-items:center;justify-content:center;font-size:36px;margin:0 auto 24px}
    ...
    .btn-start{background:#29b6e8;color:#fff}
    .btn-start:hover{background:#1da8d8;box-shadow:0 0 24px rgba(41,182,232,0.4)}
    ...
    .vol-bar span{width:4px;background:#29b6e8;border-radius:2px;transition:height 0.1s}
  </style>
</head>
<body>
  <div class="card">
    ...
    <h1>Talk to Maya</h1>
    <div class="sub">AI Dive Advisor · FlyingFish Scuba School, Goa</div>
  ...
  <script>
    ...
    function attachAudioTrack(track) {
      // Attach a remote audio track to a new <audio> element for playback
      if (track.kind === LivekitClient.Track.Kind.Audio) {
        ...
        console.log('[DEMO] Agent audio track attached and playing');
      }
    }
    ...
    async function startCall() {
      ...
      try {
        ...
        // Listen for remote audio tracks from the AI agent
        room.on(LivekitClient.RoomEvent.TrackSubscribed, (track, publication, participant) => {
          console.log('[DEMO] Track subscribed:', track.kind, 'from', participant.identity);
          attachAudioTrack(track);
        });

        await room.connect(res.url, res.token, {autoSubscribe: true});

        // Also attach any tracks already subscribed at connect time
...
try:
    ...
    # Safely create metrics (handles reload=True re-imports without crashing)
    def _get_or_create_metric(cls, name, *args, **kwargs):
        """Return existing collector or create a new one."""
        try:
            ...
            for collector in list(REGISTRY._names_to_collectors.values()):
                if hasattr(collector, '_name') and collector._name == name:
                    return collector
            # Fallback: unregister and re-create
            try:
  ...
  <style>
    *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
    :root {{
      ...
      --border: #2a3448;
      --accent: #29b6e8;
      --accent-glow: rgba(41,182,232,0.18);
      --text: #e2e8f0;
      --muted: #8892a4;
      --green: #22c55e;
      --red: #ef4444;
    ...
    .sidebar-brand .logo {{
      width: 32px; height: 32px; background: linear-gradient(135deg,#3dbf7f,#29b6e8);
    ...
    .btn-primary:hover {{ background: #1da8d8; box-shadow: 0 0 16px var(--accent-glow); }}
    ...
    .cal-cell:hover {{ border-color: var(--accent); background: var(--accent-glow); transform: scale(1.03); box-shadow: 0 4px 20px rgba(41,182,232,0.15); }}
    ...
    .stat-card:hover {{ transform: translateY(-3px); box-shadow: 0 8px 30px rgba(41,182,232,0.12); }}
...
<nav id="sidebar">
  <div class="sidebar-brand">
    ...
    <div>
      <div class="brand-text">FlyingFish Scuba</div>
      <div class="brand-sub">AI Voice Agent</div>
    </div>
  </div>
  ...
  <div id="page-agent" class="page">
    ...
    <div class="section-card">
      ...
      <div class="form-group">
        ...
        <input type="text" id="first_line" value="{config.get('first_line', '')}" placeholder="Namaste! This is Aryan from FlyingFish AI...">
  ...
  try {{
    ...
    tbody.innerHTML = contacts.map(c => `
      <tr style="border-bottom:1px solid var(--border);transition:background 0.12s;" onmouseover="this.style.background='rgba(255,255,255,0.025)'" onmouseout="this.style.background=''">
        ...
        <td style="padding:14px 16px;text-align:center;"><span style="background:rgba(41,182,232,0.12);color:var(--accent);padding:3px 10px;border-radius:20px;font-size:12px;font-weight:700;">${{c.total_calls}}</span></td>
...
function renderLangGrid() {{
  ...
  grid.innerHTML = Object.entries(LANG_PRESETS).map(([id, p]) => `
    <div onclick="selectLangPreset('${{id}}')" style="
      background:${{id===currentLangPreset ? 'rgba(41,182,232,0.15)' : 'var(--bg)'}};
